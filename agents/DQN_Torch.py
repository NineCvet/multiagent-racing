...
class DQN:
    ...
    def preprocess_state(self, state):
        ...
        Handles both dictionary, numpy array and tensor inputs.
        :param state: Dictionary, numpy array or tensor from the environment observation.
    ...
    def train(self):
        ...
        for state, action, reward, next_state, done in minibatch:
            ...
            target_motor_q = reward if done else reward + self.discount_factor * torch.max(future_q_values[0][0])
            target_steering_q = reward if done else reward + self.discount_factor * torch.max(future_q_values[0][1])

            current_q_values = self.model(state.unsqueeze(0))
            current_q_values = current_q_values.clone().detach()
            current_q_values[0][0] = target_motor_q
            current_q_values[0][1] = target_steering_q

            states.append(state)
            target_q_values.append(current_q_values.squeeze(0))

        states = torch.stack(states).to(self.device)
        target_q_values = torch.stack(target_q_values).to(self.device)
